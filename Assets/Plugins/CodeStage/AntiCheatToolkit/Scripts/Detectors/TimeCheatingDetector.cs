...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class TimeCheatingDetector : ActDetectorBase
	{
		...
		private const string FINAL_LOG_PREFIX = ACTkConstants.LOG_PREFIX + COMPONENT_NAME + ": ";
		...
		private void StartDetectionInternal(UnityAction callback, int checkInterval)
		{
			...
			{
				Debug.LogWarning(
					FINAL_LOG_PREFIX + "disabled but StartDetection still called from somewhere (see stack trace for this message)!",
					this);
				return;
			}
		...
		public void GetOnlineTimeAsync(string server, Action<double> callback)
		{
			try
			{
				...
				{
					Debug.Log(FINAL_LOG_PREFIX + "Could not resolve IP from the host " + server + " =/");
					callback(-1);
					return;
				}
		...
		private void OnSockedConnected(object sender, SocketAsyncEventArgs e)
		{
			...
			{
				Debug.Log(FINAL_LOG_PREFIX + "Could not get NTP time from " + targetHost + " =/\n" + e);
		...
		public static void StartDetection()
		{
			Debug.LogError(FINAL_LOG_PREFIX + "is not supported on selected platform or disabled with ACTK_PREVENT_INTERNET_PERMISSION conditional!");
		}
